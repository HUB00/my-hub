...
-- Charger config.json
local raw = get(CONFIG_URL)
if not raw then
    warn("[MyHub] Failed to load config.json")
    return
end

local config = HttpService:JSONDecode(raw)

-- Notification pour l'utilisateur (EN)
pcall(function()
    game.StarterGui:SetCore("SendNotification", {
        Title = "MyHub Loaded",
        Text = "Links successfully loaded from config.json.",
        Duration = 5
    })
end)

print("[MyHub] Discord: " .. config.discord)
print("[MyHub] Roblox Profile: " .. config.roblox_profile)
-- (cette zone est libre pour ton code)
...
makeSubtitle(keylessFrame, "Fast access with Roblox ⏹️")
...
makeSubtitle(withKeyFrame, "Community access with Discord 💬")
...
print("[MyHub] Main script is now running!")
